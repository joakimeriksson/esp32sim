...
impl SocBus {
    ...
    fn tlb_fill(&mut self, addr: u32) -> Option<TlbEntry> {
        ...
        let region = |lo: u32, hi: u32, src: u8, w: bool| -> TlbEntry {
            ...
            TlbEntry { lo: lo_, hi: hi_, base: std::ptr::null_mut(), off: lo_ - lo, vbase: 0, src: src as u32, writable: w as u16, code: 1, ..TlbEntry::EMPTY }
        };
        let mut e = match addr {
            ...
            DBUS_LOW..=0x3DFF_FFFF | IBUS_LOW..=0x43FF_FFFF => {
                ...
                TlbEntry { lo: page, hi: page + 0x10000, base: std::ptr::null_mut(), off: off as u32, vbase: 0, src: src as u32, writable: w as u16, code: 1, ..TlbEntry::EMPTY }
            }
            _ => return None,
        };

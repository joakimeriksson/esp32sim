...
async function runManifest(name) {
  ...
  if (w.esp32sim_profile_report) { const at = logs.length; w.esp32sim_profile_report(emu); for (const l of logs.slice(at)) for (const line of l.split('\n')) if (/ex153|wasm-region|^core=|wasm-profile\]/.test(line)) console.log(line); console.log('jit_insns', w.esp32sim_block_jit_insns(emu), 'insns', insns, 'jitstats', JSON.stringify(blockJit.stats)); }
  w.esp32sim_delete(emu);
  const wall = (Date.now() - t0) / 1000;

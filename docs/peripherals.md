...
| GDMA | 0x6003F000 | partial | out-channels (I2S0/I2S1) and in-channels (CAM); memory-to-memory channel pairs (MEM_TRANS_EN — `esp_async_memcpy`), copied whole in one round with the IN descriptors written back (length, owner, SUC_EOF) and IN_SUC_EOF_DES_ADDR set; descriptor walk, DONE/EOF/TOTAL_EOF |

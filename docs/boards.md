...
| Light Grid V1.1 3×3, ×2 | port 7 GPIO 6, port 11 GPIO 43, via RMT | `Ring::from_bits` per grid; the RMT channel is matched to a pin through the GPIO matrix, since the Arduino driver takes a fresh channel per refresh The module is mounted anti-transposed to the firmware's row-major chain (chain cell (row, col) is glass cell (2 − col, 2 − row)), so `Ring::grid` reports the glass: the SID VU's per-voice chain columns show as physical rows |

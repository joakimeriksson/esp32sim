...
| Light Grid V1.1 3×3, ×2 | port 7 GPIO 6, port 11 GPIO 43, via RMT | `Ring::from_bits` per grid; the RMT channel is matched to a pin through the GPIO matrix, since the Arduino driver takes a fresh channel per refresh. The chain is column-serpentine on the glass (connector at the bottom: down the left column, up the middle, down the right — measured LED by LED on the board with the firmware's `set_grid_pixel`), and `Ring::grid` reports the glass cell, not the chain index |

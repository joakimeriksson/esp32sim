...
const root=process.cwd(), assets=root+'/candidate/web/wasm/fw/public';
const args=['--rom',root+'/../assets/pocket-tank/rom.elf','--board','waveshare-lcd4b','--boot','rom','--no-dump','--console','usb','--flash-mb','16','--psram-mb','8','--bootloader',assets+'/panel-bootloader.bin','--ptable',assets+'/panel-ptable.bin','--app',assets+'/panel-demo.bin','--flash-at','0x610000='+assets+'/energydata.json','--script',assets+'/panel-sid.txt','--max-seconds','7'];
const sha=x=>createHash('sha256').update(x).digest('hex');
const [baselineBinary,candidateBinary,outputName,pairArg='2']=process.argv.slice(2);const pairCount=Number(pairArg); const bins={baseline:baselineBinary,candidate:candidateBinary};
const inputs=Object.fromEntries([args[1],...['panel-bootloader.bin','panel-ptable.bin','panel-demo.bin','energydata.json','panel-sid.txt'].map(n=>assets+'/'+n)].map(n=>[n,sha(fs.readFileSync(n))]));
const identities=Object.fromEntries(Object.entries(bins).map(([n,p])=>[n,{path:p,sha256:sha(fs.readFileSync(p))}]));
const out=root+'/runs/'+outputName;fs.mkdirSync(out);
...
const rows=[];
for(let pair=1;pair<=pairCount;pair++)for(const arm of pair%2?['baseline','candidate']:['candidate','baseline']){
...
const medians=Object.fromEntries(Object.keys(bins).map(arm=>[arm,rows.filter(r=>r.arm===arm).map(r=>r.wallSeconds).sort((a,b)=>a-b).slice(Math.floor((pairCount-1)/2),Math.floor(pairCount/2)+1).reduce((s,x,_,a)=>s+x/a.length,0)]));

...
Updated 2026-09-20 14:49:35. One pair per pass, alternating base/candidate, serialized. "red%" = wall-time reduction vs base (positive = faster).
...
| CONTROL-aa--c007 | 76.90 | 73.71 | +4.14 |
| CONTROL-aa--c008 | 77.59 | 75.75 | +2.37 |
...
| kernel-s1-f2717977-pocket-tank | kernel | +11.72 | +12.02, +11.43 | 72.6->63.9, 76.1->67.4 | True | - | EX156 s1: guarded block body: resumed/cut block calls enter via br_table and pay one STOP compare per instruction instead of checked-body entry/budget/LEND/pending checks; LEND hinted at compile time gives one static loop-end site. Census (12 guest s): checked body carried 1.44G of 4.28G insns. Commit 94f2cef4 on base. |
| coverage-s1-be80f476-pocket-tank | coverage | +11.09 | +10.72, +11.46 | 73.9->66.0, 76.8->68.0 | True | - | EX155 s1: inline WUR/RUR SAR_BYTE + ee.src.q[.qup], ee.vsr/vsl.32, vsubs/vadds s8/s16, vmin/vmax: core-1 4-bit unpack kernel compiles (interp 208.9M->14.1M insns). On plain base. |
...
| turnover-s1s2-6ddaceff-pocket-tank | turnover | +8.15 | +8.15 | 77.7->71.4 | True | - | auto-queued by exact-all.sh after EXACT ok; artifact turnover-s1s2, see its agent note/commits |
...
| turnover-s1-b505b798-pocket-tank | turnover | +5.68 | +4.45, +6.91 | 72.9->69.6, 75.4->70.2 | True | - | EX152 s1: run_many - consecutive blocks inside Cpu (block.rs run_blocks) instead of returning to machine step_blocks per block; lean flag when no probes/stubs/observers; alone on base |
...
| turnover-s1s2s3-16a3e375-pocket-tank | turnover | +4.95 | +4.95 | 75.9->72.2 | True | - | auto-queued by exact-all.sh after EXACT ok; artifact turnover-s1s2s3, see its agent note/commits |
...
| turnover-s3-b3fefc2f-pocket-tank | turnover | +1.76 | +1.76 | 74.7->73.4 | True | - | auto-queued by exact-all.sh after EXACT ok; artifact turnover-s3, see its agent note/commits |
...
| spills-m2-8ef8ff94-pocket-tank | spills | -0.82 | -0.82 | 74.0->74.6 | True | - | auto-queued by exact-all.sh after EXACT ok; artifact spills-m2, see its agent note/commits |
...
| spec-tax3-30114a9f-pocket-tank | spec | -1.19 | -1.19 | 75.0->75.9 | True | - | auto-queued by exact-all.sh after EXACT ok; artifact spec-tax3, see its agent note/commits |
...
| spills-m1-f0efcd1b-pocket-tank | spills | -2.14 | -2.14 | 73.0->74.5 | True | - | auto-queued by exact-all.sh after EXACT ok; artifact spills-m1, see its agent note/commits |
...
| turnover-s2-c58b96eb-pocket-tank | turnover | -4.61 | -4.61 | 73.4->76.8 | True | - | auto-queued by exact-all.sh after EXACT ok; artifact turnover-s2, see its agent note/commits |
| spec-tax7-70697d75-pocket-tank | spec | -5.31 | -5.31 | 73.7->77.6 | True | - | auto-queued by exact-all.sh after EXACT ok; artifact spec-tax7, see its agent note/commits |
| spec-tax4-7ea8bc5c-pocket-tank | spec | -6.98 | -6.98 | 71.1->76.1 | True | - | auto-queued by exact-all.sh after EXACT ok; artifact spec-tax4, see its agent note/commits |

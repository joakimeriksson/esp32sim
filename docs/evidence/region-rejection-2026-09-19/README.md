...
Candidate: `codex/region-rejection-0919`, based on `d549d4b27b927e7bd32bd2e4e9cc4188738c8e03`. Eight added production lines in `xtensa-lx7/src/jit/wasm.rs`. **Unbuilt and unmeasured; not adopted.**
...
An independent static review accepted this argument, specifically checking owner replacement, stale-page invalidation, cache reindexing and active-loop admission. `git diff --check` passes. No executable validation was run during the coordinated CPU quiet window.

Validation pending: run `tools/wasm-jit-test.sh` after the coordinated CPU quiet window. Its existing `wasm_tests/regions.rs` differential programs vary budgets, head/interior probes, hardware loops, code changes and timer deadlines. Then build a default candidate and screen equal-work pocket-tank against the exact d549d4b2 baseline, checking instruction totals and console hashes before interpreting time. No throughput claim is supported yet.

...
The review fixes pin the presence of direct forward branches and both explicit and hardware-loop backedges. Test-only emitter counters are sampled around each existing differential program, so reverting to the semantically correct dispatcher path fails the corresponding assertion. Builds without the `wasm-jit-tests` feature omit these counters. Region generation also asserts that chunk heads are unique, the invariant that makes the head count match the dispatch-block nesting. The hardware-loop encoding check now includes offset 5.

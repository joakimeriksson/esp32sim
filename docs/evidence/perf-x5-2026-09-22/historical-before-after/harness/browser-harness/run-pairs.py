...
def capture(arm, run, chrome, expected, workload='tinydraw'):
    ...
    with (run / 'server.log').open('w') as server_log, (run / 'chrome.log').open('w') as chrome_log, (run / 'capture.log').open('w') as capture_log:
        try:
            ...
            subprocess.run(['node', str(HERE / 'capture-battery.mjs'), f'http://127.0.0.1:{port}/battery.html', str(run), str(debug)], stdout=capture_log, stderr=subprocess.STDOUT, check=True, timeout=1860)
        finally:

...
onmessage=async ({data})=>{
  try {
    ...
    const capabilities = await (await fetch('/capability.json')).json();
    ...
    }, e=>postMessage(e.type === 'result' ? {...e, provenance} : e), data.jit!==false, data.chain===true, workload, {mode:capabilities.mode});

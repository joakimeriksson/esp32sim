...
def read_run(directory, *, legacy=False):
    ...
    if (not version.get('Browser', '').startswith('Firefox/')
            or 'Firefox/' not in version.get('User-Agent', '')
            or version.get('Engine') != 'SpiderMonkey'
            or capture.get('captureTransport') != 'Firefox ordinary browser page'):
        raise ValueError(f'{directory}: expected a Firefox ordinary-page timing capture')
    ...
    return {
        ...
        'v8': None,  # Firefox uses SpiderMonkey; no engine version was measured.
    }

...
def read_run(directory, *, legacy=False):
    ...
    if (not version.get('Browser', '').startswith('Safari/')
            or 'Safari/' not in version.get('User-Agent', '')
            or version.get('Engine') != 'JavaScriptCore'
            or capture.get('captureTransport') != 'Safari ordinary browser page'):
        raise ValueError(f'{directory}: expected a Safari WebDriver timing capture')
    ...
    return {
        ...
        'v8': None,  # Safari uses JavaScriptCore; no engine version was measured.
    }

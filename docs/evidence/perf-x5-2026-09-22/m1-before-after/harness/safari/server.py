#!/usr/bin/env python3
"""Safari ordinary-page fallback: no WebDriver, AppleScript or permission changes.

Start after Chrome finishes. Open the printed token URL in Safari. Each arm uses
...
PAGE = r'''<!doctype html><meta charset="utf-8"><title>M1 Safari paired benchmark</title>
...
const timeout=setTimeout(()=>finish({type:'error',line:'Safari battery timeout after 660 seconds'}),660000);
async function finish(result){
  ...
  try {
    ...
    if(answer.done){status.textContent='All Safari pairs completed. Results saved on disk.';return}
...
if(hidden)finish({type:'error',line:'The Safari benchmark page must be visible'});
...
def main():
            ...
            with lock:
                ...
                try:
                    ...
                    if not match or any(s in ua for s in ('Chrome/', 'Chromium/', 'Edg/')):
                        raise ValueError('This campaign requires Safari')
                    raw = {'captureMode': 'timing', 'captureTransport': 'Safari ordinary browser page',
                           'version': {'Browser': 'Safari/' + match[1], 'User-Agent': ua, 'Engine': 'JavaScriptCore'}, 'result': data['result']}
                    pairs.write_json(run / 'result.json', raw)
                    pairs.write_json(run / 'events.json', data['events'])
                    pairs.write_json(run / 'capture.json', {'mode': 'timing', 'browserFamily': 'Safari',
                    ...
                    if data['hidden']:
                        raise ValueError('Safari page was hidden during this arm')
                    ...
                    row = {'pair': item['pair'], 'arm': item['arm'], 'consoleSha256': checked['consoleSha256'],
                        'browser': checked['browser'], 'v8': None, 'engine': 'JavaScriptCore',
    ...
    print(f'Open in Safari after Chrome finishes: {server_origin}/?token={token}&index=0', flush=True)

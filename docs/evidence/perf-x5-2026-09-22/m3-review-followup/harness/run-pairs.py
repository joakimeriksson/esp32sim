...
def prepare(out, name, tree, wasm, assets, rustflags=None):
    ...
    arm = out / name
    arm.mkdir()
    tracked = set(str(p.relative_to(tree)) for p in tree.rglob('*') if p.is_file())
    ...
    record = {'tree': str(tree), 'commit': (tree / 'source-revision.txt').read_text().strip(),
    ...
    (arm / 'source.patch').write_text('Runtime snapshot only; artifact source revision is in artifactBuild.')
    if wasm is None:
        env = os.environ.copy()
...
def capture(arm, run, chrome, expected, workload='tinydraw'):
    ...
    # Preserve scratch browser profile under task retention policy.

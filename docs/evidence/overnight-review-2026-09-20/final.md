...
The final build source is `946497a3`. The compared WASM artifacts are identified by their source metadata and SHA-256 in the [baseline record](runs/final-pocket-tank/baseline/build.json) and [candidate record](runs/final-pocket-tank/candidate/build.json). Native executable hashes and command lines are in its [summary](runs/final-native-pocket-tank/summary.json). Builds and timed arms ran serially. Run `node final-audit.mjs .` from this directory to validate the retained result contracts. Source path and account labels throughout this evidence directory are normalized to `/Users/user`; artifact, input and source SHA-256 values are unchanged.

...
T=/Users/sarah/src/a/esp32sim/work/night-timed
...
import json,sys
a=json.load(open('/Users/sarah/src/a/esp32sim/work/night-run/assets.json')); a['wasm']=sys.argv[1]
json.dump(a,open(sys.argv[2]+'/assets.json','w'))
P
cd $T
uv run --no-project --python /Users/sarah/src/a/esp32sim/.venv/bin/python tools/browser-benchmark/serve.py "$OUT/assets.json" --port 8797 > "$OUT/server.log" 2>&1 & S=$!
